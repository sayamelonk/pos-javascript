    ...
    if (token) {
      ...
      try {
        ...
        const response = await Api.get(
          `/api/sales?start_date=${startDate}&end_date=${endDate}`
        );

        // assign response data to state "sales"
        setSales(response.data.data.sales);
        setSalesTotal(response.data.data.total);
      } catch (error) {
        ...
        <div className="container-xl">
          <div className="row g-2 align-items-center">
            <div className="col">
              ...
              <h2 className="page-title">REPORT SALES</h2>
            </div>
          </div>
        </div>
      </div>
                      ...
                      <table className="table table-bordered table-hover">
                        ...
                        <tbody>
                          {sales.map((sales, index) => (
                            <tr key={index}>
                              <td>{formatDate(sales.created_at)}</td>
                              <td>{sales.invoice}</td>
                              <td>{sales.cashier.name}</td>
                              <td>{sales.customer?.name ?? "Umum"}</td>
                              <td className="text-end">
                                {moneyFormat(sales.grand_total)}
                              </td>
                            </tr>
                          ))}
                          <tr>
